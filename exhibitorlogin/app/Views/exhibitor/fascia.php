...
$exhibitorType = $exhibitor['exhibitor_type'] ?? 'Domestic';
$isDomestic = ($exhibitorType === 'Domestic');
...
if ($isDomestic) {
    ...
    $currencyCode = 'INR';
    $currencyBadge = 'INR';
    $badgeClass = 'currency-inr';
    $iconClass = 'bi-currency-rupee';
    $exhibitorBadge = 'Domestic';
    $badgeType = 'badge-domestic';
} else {
    $currencySymbol = '$';
    $currencyCode = 'USD';
    $currencyBadge = 'USD';
    $badgeClass = 'currency-usd';
    $iconClass = 'bi-currency-dollar';
    $exhibitorBadge = 'International';
    $badgeType = 'badge-international';
}
?>
    ...
    <div class="fascia-wrapper">
        ...
        <div class="fascia-card">
            <div class="fascia-card-head">
                <div class="fascia-card-head-top">
                    <div>
                        ...
                        <h4 class="fascia-title" id="fasciaTitle">Raw Space &amp; Stand Details</h4>
                ...
                <p id="fasciaSubtitle">Tell us how your raw space stand will be designed and constructed so we can review the layout, coordinate approvals, and ensure a smooth build-up before the event.</p>
            </div>
            <div class="fascia-form-layout">
                ...
                <form id="rawForm" method="post" style="display:block;" enctype="multipart/form-data">
...
<script>
$(function() {
    const exhibitorType = <?= json_encode($exhibitorType) ?>;
    const isDomestic = exhibitorType === 'Domestic';
    const isInternational = exhibitorType === 'International';
    let currencySymbol = <?= json_encode($currencySymbol) ?>;
    let currencyCode = <?= json_encode($currencyCode) ?>;
    ...
    function updateElectricityPriceNote(quantity) {
        ...
        if (!electricityItemLoaded || !ELECTRICITY_RATE_PER_KW) {
            $note.html(`
        ...
        const exhibitorTypeText = isDomestic ? 'Domestic' : (isInternational ? 'International' : '');
        $note.html(`
            <i class="bi bi-info-circle"></i>
            ${quantity} KW × ${currentSymbol}${ELECTRICITY_RATE_PER_KW.toFixed(2)} = ${currentSymbol}${price.toFixed(2)} 
            <span style="font-weight:400;color:#6c757d;font-size:0.8rem;">
                (${exhibitorTypeText} - ${currencyCode})
            </span>
        `).show();
    }

    function getElectricityRateForExhibitor(baseRate, type) {
    ...
    function fetchElectricityItem() {
        ...
        return $.ajax({
            ...
            if (response && response.status && response.data) {
                ...
                let baseRate = 0;
                if (response.data.rate_per_kw) {
                    baseRate = parseFloat(response.data.rate_per_kw);
                } else if (response.data.rate) {
                    baseRate = parseFloat(response.data.rate);
                } else if (response.data.price) {
                    baseRate = parseFloat(response.data.price);
                } else if (response.data.amount) {
                    baseRate = parseFloat(response.data.amount);
                }
                ELECTRICITY_RATE_PER_KW = getElectricityRateForExhibitor(baseRate, exhibitorType);
                ...
                const currentValue = $('#electricityRequirement').val();
                if (currentValue && parseInt(currentValue) > 0) {
                    handleElectricityInputChange(parseInt(currentValue));
                }
            } else {
    ...
    function getCartPageUrl() {
        ...
        return `${base}/additional-furniture?view=cart`;
    }

    function notifyElectricityAddedToCart(quantity) {
        const price = quantity * ELECTRICITY_RATE_PER_KW;
        const exhibitorTypeText = isDomestic ? 'Domestic' : (isInternational ? 'International' : '');
        const message = `Electricity (${quantity} KW, ${currencySymbol}${price.toFixed(2)}) added to your cart. Rate: ${currencySymbol}${ELECTRICITY_RATE_PER_KW.toFixed(2)}/KW (${exhibitorTypeText}). Please visit the cart page to complete payment.`;
                    ...
                    if (isRawSpaceSubmit) {
                        ...
                        cartRequest.then(function(cartResponse) {
                            if (cartResponse && cartResponse.status) {
                                const price = electricityQuantity * ELECTRICITY_RATE_PER_KW;
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Details Submitted!',
                                    // html: `
                                    //     <div style="text-align: left; padding: 10px 0;">
                                    //         <p><strong>Electricity charges:</strong> ${currencySymbol}${price.toFixed(2)}</p>
                                    //         <p><strong>Quantity:</strong> ${electricityQuantity} KW</p>
                                    //         <p><strong>Rate:</strong> ${currencySymbol}${ELECTRICITY_RATE_PER_KW.toFixed(2)}/KW</p>
                                    //         <hr>
                                    //         <p class="text-muted" style="font-size: 0.9rem;">Charges have been added to your cart.</p>
                                    //     </div>
                                    // `,
                                    showDenyButton: true,
